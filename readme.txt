...
= 1.0.6 =
* 업데이트 감지 수정 — GitHub Releases가 없고 태그(tag)만 있어도 새 버전을 자동 감지
* 업데이트 완료 후 캐시 자동 삭제로 다음 버전 체크 즉시 반영
* 스니펫 데이터 보호 — 플러그인 삭제 시 기본적으로 데이터 보존 (실수로 삭제 방지)
* fix_source_dir 안정화 — 업데이트 후 플러그인 폴더명 교정 시 충돌 오류 수정
